...
class AdaptiveGST(object):
    def __init__(self, p0, lift, alpha=0.05, power=0.8, alternative="two-sided", outcome_type='bin',
                 control=[], test=[], decision_check_flg=False, stop_flg=False, peeking_array=[],
                 iuse=3, phi=1, sigma=None):
    ...
    def z_statistics_camulative(self, test, control):
        ...
        if self.outcome_type == 'bin':
            ...
            z_value = (mean_t - mean_c) / np.sqrt(abs((std_t ** 2) / n_t + (std_c ** 2) / n_c))
    ...
    def sample_size(self, ratio=1):
        ...
        if effect > 0:
            ...
            if self.outcome_type == 'bin':
                pooled_prob = (p0 * (1 - p0)) + (p1 * (1 - p1))
                sample_size = int(np.ceil(((z_alpha + z_beta) ** 2 * pooled_prob) / (mde ** 2)))
                ...
                raise ValueError("Wrong outcome type")
    ...
    def sample_ratio(self, control, test):
        ...
        sample_ratio = n_min / (self.sample_size())
